...
public class RestauranteNotificacionService {
    ...
    @Autowired
    private ContenidosRestaurantesRepository contenidosRepository;
     ...
     */
    public boolean notificarRestaurante(ClickBean click) {
        try {
            // 1. Obtener información del restaurante (usando IDs directamente con JDBC)
            Integer nroRestaurante = click.getNroRestaurante();
            RestauranteBean restaurante = restauranteRepository.findById(nroRestaurante)
                    .orElseThrow(() -> new RuntimeException("Restaurante no encontrado: " + nroRestaurante));

            // 2. Obtener el contenido promocional usando los IDs
            ContenidoIdBean contenidoId = new ContenidoIdBean();
            contenidoId.setNroRestaurante(click.getNroRestaurante());
            contenidoId.setNroIdioma(click.getNroIdioma());
            contenidoId.setNroContenido(click.getNroContenido());

            ContenidoBean contenido = contenidosRepository.findById(contenidoId)
            ...
            notificacionData.put("contenidoPromocional", contenido.getContenidoPromocional());

            // 3. URL del servicio REST del restaurante (simulado)
            // En producción, esta URL vendría de la configuración o de la base de datos
            String restauranteUrl = obtenerUrlRestaurante(restaurante);
            ...
            String endpoint = UriComponentsBuilder.fromHttpUrl(restauranteUrl)
                    .path("/notificaciones/click")
                    .toUriString();
     ...
     */
    private String obtenerUrlRestaurante(RestauranteBean restaurante) {
        if (restaurante.getUrlNotificacion() != null && !restaurante.getUrlNotificacion().isBlank()) {
            return restaurante.getUrlNotificacion();
        }
